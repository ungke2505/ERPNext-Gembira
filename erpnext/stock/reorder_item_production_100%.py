			...
			try:
				...
				for warehouse, warehouse_items in items_by_warehouse.items():
					...
					for d in warehouse_items:
						...
						uom = item.stock_uom
						kuadran_gg = item.custom_grosir_gorontalo
						...
						# Get the custom_area of the warehouse
						custom_area = frappe.db.get_value("Warehouse", warehouse, "custom_area")


						# Set the target warehouse based on the custom_area
						if custom_area == "GORONTALO":
							target_warehouse = default_warehouse
						elif custom_area == "MANADO":
							target_warehouse = "DC MANADO GUDANG 1 - PTGGS"
						else:
							target_warehouse = warehouse
						# If the target warehouse is same with target_warehouse
						...
						mr.append(
							"items",
							{
								...
								"custom_kuadran_gg": kuadran_gg,
								"schedule_date": add_days(nowdate(), cint(item.lead_time_days)),
								"qty": qty,
								"uom": uom,
								"stock_uom": item.stock_uom,
								"warehouse": warehouse,  # Use the default warehouse
								"from_warehouse": target_warehouse,  # Use the default warehouse as the source warehouse
