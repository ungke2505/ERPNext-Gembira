...
class SalesInvoice(SellingController):
	...
	def validate(self):
		...
		#Program Diskon Darurat
		#if self.is_return:
			#return
		#last_invoice = frappe.get_list("Sales Invoice", filters={"customer": self.customer, "docstatus": 1}, order_by="creation desc", limit_page_length=1)
		#if last_invoice:
			#last_invoice_doc = frappe.get_doc("Sales Invoice", last_invoice[0].name)
			#if last_invoice_doc.custom_cashback > 0:
				#self.discount_amount = last_invoice_doc.custom_cashback
			#else:
				#self.discount_amount = 0
			# Set the discount_amount with custom_cashback from last invoice
			# self.discount_amount = last_invoice_doc.custom_cashback or 0
	...
	def on_submit(self):
		if "Langganan" in self.customer_group or "Grosir" in self.customer_group:
			num_lottery_numbers = math.floor(self.grand_total / 100000) * 3
			suffix = 'MM'
			if 'Toko GHW' in self.cost_center:
				suffix = 'GHW'
			elif 'Toko GHM' in self.cost_center:
				suffix = 'GHM'
			elif 'Toko GGS' in self.cost_center:
				suffix = 'GGS'
			elif 'Toko GHI' in self.cost_center:
				suffix = 'GHI'
			elif 'Toko GHTM' in self.cost_center:
				suffix = 'GHTM'
			elif 'Toko GHSR' in self.cost_center:
				suffix = 'GHSR'
			elif 'Toko GHL' in self.cost_center:
				suffix = 'GHL'
			elif 'Toko GRA' in self.cost_center:
				suffix = 'GRA'
			elif 'Toko GHDS' in self.cost_center:
				suffix = 'GHDS'
			elif 'Toko GHTI' in self.cost_center:
				suffix = 'GHTI'
			elif 'Toko GHTU' in self.cost_center:
				suffix = 'GHTU'
			elif 'Toko GHPD' in self.cost_center:
				suffix = 'GHPD'
			elif 'Toko GHKR' in self.cost_center:
				suffix = 'GHKR'
			custom_undian_gembira = ['UND-' + str(random.randint(1000000, 9999999)) + '.' + suffix + '.' for _ in range(num_lottery_numbers)]
			lottery_numbers_str = ', '.join(custom_undian_gembira)
			self.db_set('custom_undian_gembira', lottery_numbers_str)
				# Program dicount darurat
			#if "Grosir" in self.customer_group and self.customer_group != "Grosir Manado" and self.grand_total >= 5000000:
				#cashbacks = self.grand_total * 0.05  # Calculate 5% of grand_total
				#self.db_set('custom_cashback', cashbacks)
			#elif "Langganan" in self.customer_group and self.customer_group != "Langganan Manado" and self.grand_total >= 100000:
				#cashback = self.grand_total * 0.05  # Calculate 5% of grand_total
				#self.db_set('custom_cashback', cashback)
			self.validate_pos_paid_amount()
   			# Split the custom_undian_gembira string into a list
			lottery_numbers_list = self.custom_undian_gembira.split(', ')
                        # Count the number of elements in the list
			total_lottery_numbers = len(lottery_numbers_list)
                        # Set the total_nomor_undian field in the database
			self.db_set('total_nomor_undian', total_lottery_numbers)
		self.validate_pos_paid_amount()
